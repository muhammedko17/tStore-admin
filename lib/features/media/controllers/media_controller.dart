...
class MediaController extends GetxController {
  ...
  void uploadImagesConfirmation() {
    if (selectedPath.value == MediaCategory.none) {
      TNotificationOverlay.info(
          context: Get.context!, title: 'Select Folder', subTitle: 'Please select the Folder in Order to upload the Images');
      return;
    }

    if (mediaFolders[selectedPath.value] == null) {
      TNotificationOverlay.warning(
          context: Get.context!, title: 'Folder not found', subTitle: 'You need to upload some images from your local device.');
      return;
    }
    ...
    TDialogs.defaultDialog(
      ...
      content: Text.rich(
        TextSpan(
          children: [
            TextSpan(text: 'Are you sure you want to upload all the Images in '),
  ...
  removeCloudImage(ImageModel image) async {
    try {
      ...
      TNotificationOverlay.success(
        context: Get.context!,
        title: 'Image Deleted',
        subTitle: 'Image successfully deleted from your cloud storage',
      );
    } catch (e) {
      TFullScreenLoader.stopLoading();
  ...
  void uploadImagesPopup(BuildContext context) {
    Get.defaultDialog(backgroundColor: Colors.white, content: const MediaUploader(), barrierDismissible: true, title: 'Upload Images');
  }
}

...
class CreateCategoryController extends GetxController {
  ...
  Future<void> createSubCategory() async {
    try {
      // Check Internet Connectivity
      final isConnected = await NetworkManager.instance.isConnected();
      if (!isConnected) throw 'No Internet Connection. Please try back when you have active internet';

      // Form Validation
      if (!subCategoryFormKey.currentState!.validate()) return;

      // Make sure parentId is selected, else return
      if (selectedParent.value.id.isEmpty) throw 'To create a Sub Category, Choose Parent.';
      ...
      TNotificationOverlay.success(context: Get.context!, title: 'Sub Category Created', duration: Duration(seconds: 3));

...
class EditCategoryController extends GetxController {
  ...
  Future<void> updateCategory(CategoryModel category) async {
    try {
      ...
      TNotificationOverlay.success(context: Get.context!, title: 'Category Updated', duration: Duration(seconds: 3));
  ...
  Future<void> updateSubCategory(CategoryModel category) async {
    try {
      // Check Internet Connectivity
      final isConnected = await NetworkManager.instance.isConnected();
      if (!isConnected) throw 'Check your internet Connection';

      // Form Validation
      if (!formKey.currentState!.validate()) throw 'Invalid Form Details';

      if (selectedParent.value.id.isEmpty) throw 'Parent Category cannot be empty';
      ...
      // Success Message & Redirect
      TNotificationOverlay.success(context: Get.context!, title: 'Sub Category Updated', duration: Duration(seconds: 3));

...
class OrderDetailController extends GetxController {
  ...
  Future<void> init() async {
    try {
      ...
      TLoaders.errorSnackBar(title: 'Oh Snap', message: 'Unable to fetch order details. Try again.');
    }
  }
    ...
    try {
      ...
      if (!_isValidStatusTransition(order.value.orderStatus, selectedStatus.value)) {
        TLoaders.warningSnackBar(
            title: 'Invalid Status', message: 'Cannot move from ${order.value.orderStatus.name} to ${selectedStatus.value.name}');
        return;
      }
  ...
  String _generateNotificationTitle(OrderStatus status) {
    switch (status) {
      ...
      case OrderStatus.processing:
        return 'Order Update: Now Processing';
      case OrderStatus.shipped:
        return 'Order Shipped: On Its Way!';
      case OrderStatus.delivered:
        return 'Order Delivered: Enjoy Your Purchase!';
      case OrderStatus.canceled:
        return 'Order Canceled: We’re Sorry to See It Go';
      case OrderStatus.refunded:
        return 'Refund Processed: Your Refund is on the Way';
      default:
        return 'Order Update';
    }
  }
